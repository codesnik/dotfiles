...
class Prompt
  ...
  public
    ...
    def refresh
      pretty_msg("Comment", @@PROMPT, "None", @input, "Underlined", "_")
    end
...
class SpacingPrompt < Prompt
  def pruning_regex
    Regexp.new( @input.split( /\s+/ ).map{ |x| Regexp.escape(x) }.join( '.*' ) )
  end
end
